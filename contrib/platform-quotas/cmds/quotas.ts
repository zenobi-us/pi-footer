...
Footer.registerSubCommand(
  'quota',
  'Print a table of the Platform Quotas UsageTracker store',
  async (_args, ctx) => {
    const entries = Array.from(usageTracker.store.entries()).sort(([a], [b]) => a.localeCompare(b));
    ...
    if (!ctx.hasUI) {
      ...
      const header = '| provider | active | fails | updated | windows (remaining %, used, total) |';
      const divider = '| --- | --- | --- | --- | --- |';
    ...
    await ctx.ui.select(`UsageTracker store (${entries.length} entries)`, options);
  }
);
