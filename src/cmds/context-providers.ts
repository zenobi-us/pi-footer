...
export function registerContextProvidersCommand(
  ...
  pi.registerCommand("context-providers", {
    ...
    handler: async (_args, ctx) => {
      ...
      const providers = Array.from(ft.template.providers.keys() as Iterable<string>).sort(
        (a, b) => a.localeCompare(b),
      );
      const steps = Array.from(ft.template.steps.keys() as Iterable<string>).sort(
        (a, b) => a.localeCompare(b),
      );
      ...
      await ctx.ui.select(
        `Providers (${providers.length}) & Steps (${steps.length})`,
        items,
      );
    },
  });
